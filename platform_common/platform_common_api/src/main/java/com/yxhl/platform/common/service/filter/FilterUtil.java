package com.yxhl.platform.common.service.filter;
    ...
    */
    ...
    public static Wrapper parsePropertyFilterExp(String filterStr, String orderByStr) throws Exception {
        ...
        if(!filterStr.matches(expReg)) {
            throw new Exception("错误的格式，请调整如下例：id$L$3$EQ$OR");
        }
        List<String[]> exps = StringHelper.matchAll(filterStr, expReg);
        for (String[] singleExp : exps) {
            String property =singleExp[1];//StringHelper.camelToUnderline(singleExp[1]); //实体属性转换为数据库属性
            ...
            if (mt.equalsIgnoreCase("IN") || mt.equalsIgnoreCase("NOTIN") ) {
                values = com.google.common.base.Splitter.on(",").splitToList(value);
            }

            if (mt.equalsIgnoreCase("EQ")) {
                wrapper.eq(property, value);
            } else if (mt.equalsIgnoreCase("NE")) {
                wrapper.ne(property, value);
            }else if (mt.equalsIgnoreCase("GE")) {
                wrapper.ge(property, value);
            } else if (mt.equalsIgnoreCase("GT")) {
                wrapper.gt(property, value);
            } else if (mt.equalsIgnoreCase("IN")) {
                wrapper.in(property, values);
            } else if (mt.equalsIgnoreCase("LE")) {
                wrapper.le(property, value);
            } else if (mt.equalsIgnoreCase("LT")) {
                wrapper.lt(property, value);
            } else if (mt.equalsIgnoreCase("LIKE")) {
                wrapper.like(property, value);
            } else if (mt.equalsIgnoreCase("NOTLIKE")) {
                wrapper.notLike(property, value);
            }else if (mt.equalsIgnoreCase("ISNULL")) {
                wrapper.isNotNull(property);
            } else if (mt.equalsIgnoreCase("LEFT")) {
                wrapper.like(true,property, value,SqlLike.RIGHT);
            }else if (mt.equalsIgnoreCase("NOTIN")) {
                wrapper.notIn(property, values);
            }else if (mt.equalsIgnoreCase("BETWEEN")) {
            	String[] val=value.split("[,;]");
                wrapper.between(property, val[0], val[0]);
            } else if (mt.equalsIgnoreCase("EXISTS")) {
                wrapper.exists(value);
            }
            else if (mt.equalsIgnoreCase("NOTEXISTS")) {
                wrapper.notExists(value);
            }
        }
